...
class Agent:
    def __init__(
        ...
        if use_icm:
            ...
            self.icm_optimizer = optim.AdamW(
    ...
    def train(self, states, next_states, targets, actions, advs, old_policies):
        ...
        for i in range(self.epochs):
            ...
            for j in range(int(len(states) / self.batch_size)):
                ...
                loss = 0.5 * (actor_loss + 0.5 * critic_loss - 0.001 * entropy) + (1 - self.beta)*inverse_dynamics_loss + self.beta * forward_loss
                
                loss.backward()

                if self.use_icm:
                    torch.nn.utils.clip_grad_norm_(self.icm.parameters(), 0.3)
