    ...
    def _update_state(self, output: UnityRLOutputProto) -> None:
        ...
        for brain_name in self._env_specs.keys():
            if brain_name in output.agentInfos:
                ...
                # Split agents into active and terminated
                active_agents = []
                terminated_agents = []
                for agent_info in agent_info_list:
                    if agent_info.done:
                        terminated_agents.append(agent_info)
                    else:
                        active_agents.append(agent_info)
                
                # Create separate DecisionSteps and TerminalSteps
                decision_steps = steps_from_proto(
                    active_agents, self._env_specs[brain_name]
                )[0] if active_agents else DecisionSteps.empty(self._env_specs[brain_name])
                
                terminal_steps = steps_from_proto(
                    terminated_agents, self._env_specs[brain_name]
                )[1] if terminated_agents else TerminalSteps.empty(self._env_specs[brain_name])
                
                self._env_state[brain_name] = (decision_steps, terminal_steps)
            else:
